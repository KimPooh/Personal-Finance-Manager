...
- 완료 기준: PostgreSQL 배포 준비 → Vercel 배포 → 갤럭시 실기기 최종 검증 →
  포트폴리오 사례 페이지 갱신(실제 운영 URL은 비공개 유지 - 아래 2장 10번 참고).
...
| `ENCRYPTION_KEY` | 필수, **로컬에서 이미 쓰던 것과 동일한 키를 그대로 사용** | 필수, **Production과 다른 별도 키** | 필수 (기존 값 그대로) | 필드 단위 AES-256-GCM 암호화 키 (64자리 hex). Production이 로컬과 같은 키를 유지해야 아래 3장의 백업 복원이 정상 동작합니다. Preview는 실제 데이터/백업을 절대 주입하지 않으므로 별도 키를 씁니다 |
| `SESSION_SECRET` | 필수, **로컬 값을 재사용하지 않는 새 무작위 값** | 필수, **Production과도 다른 별도 무작위 값** | 필수 (기존 값 그대로) | iron-session 쿠키 암호화 키 (32자 이상). `ENCRYPTION_KEY`와 달리 환경 간 복원 호환성이 필요 없으므로, 세 환경 모두 서로 다른 무작위 값을 씁니다 |
...
6. **로그인 확인**: 배포된 URL에서 방금 만든 관리자 계정으로 실제 로그인해
   봅니다.
7. **백업 복원** (기존 데이터를 유지하고 싶은 경우에만, 아래 3장 데이터 이전 절차
   참고).
8. **데이터 검증** (아래 4장 복원 후 데이터 검증 체크리스트).
9. **갤럭시 실기기 최종 검증**: 실사용자의 안드로이드 갤럭시 폰에서 로그인 ·
   CSV/엑셀 업로드 · 핵심 기능을 확인합니다.
10. **포트폴리오 사례 페이지 갱신**: 포트폴리오 사이트(`KimPooh/portfolio-site`)의
    이 앱 케이스 스터디에 배포 완료 상태·기술 스택 설명·스크린샷을 갱신합니다.
    **실제 운영(production) URL은 비공개로 유지하고 포트폴리오에 공개 링크로
    걸지 않습니다** - 이 앱에는 실제 개인 재무 데이터가 들어가므로, URL을 아는
    누구나 접근을 시도할 수 있는 상태로 공개하지 않습니다. 공개 체험판이 필요하면
    가짜 데이터만 쓰는 별도 demo/Preview 환경을 향후 별도로 설계해야 하며, 이번
    범위에서는 만들지 않습니다.
...
- **이미 내보낸 백업 JSON 파일이 있다면**: 그 파일을 그대로 사용해 2장 실행 순서의
  7번("백업 복원") 단계로 갑니다.
...
- 마이그레이션 실패: production DB를 되돌리려 하지 말고 즉시 중단, 어떤 마이그레이션
  단계에서 실패했는지 기록 후 상의.
- 관리자 생성 실패: `createInitialAdmin`의 Serializable 트랜잭션은 DB 오류(검증
  실패, 이미 계정 존재, 직렬화 충돌 등)가 나면 원자적으로 롤백되므로, **트랜잭션
  자체가 실패한 경우**는 계정이 생성되지 않은 상태 그대로입니다 - 이때는 그냥
  `npm run create-admin`을 다시 실행하면 됩니다. 다만 트랜잭션은 실제로
  커밋(계정 생성)됐는데 그 직후 성공 메시지를 출력하는 단계에서만 스크립트가
  중단된 경우에는 계정이 이미 만들어져 있을 수 있습니다. 이 경우 무작정 재실행하면
  `accountExists` 오류가 날 수 있으므로, 재실행 전에 먼저 방금 만들려던 아이디로
  실제 로그인이 되는지(=계정이 이미 존재하는지) 확인한 뒤 필요할 때만 재실행합니다.
